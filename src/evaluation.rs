...
pub fn evaluate_material_difference(position: &Position) -> isize {
    ...
    match check_for_mate(position) {
        None => {
            if depth == 0 {
                ...
                PositionEval::Undecided(evaluate_material_difference(position) as f32)
            } else {
...
#[cfg(test)]
mod tests {
    use crate::{FinishedState, PieceColour, PositionEval, evaluation, fen};
